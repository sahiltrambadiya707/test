...
module.exports.setup = () => {
  ...
  passport.use(
    new JwtStrategy(jwtOptions, (req, jwt_payload, next) => {
      const { id, date, environment, phone, scope, email, type: type } = jwt_payload;

      const reqInfo = `REQ [${req.requestId}] [${req.method}] ${req.originalUrl}`;
      logger.info(`${reqInfo} - #JwtStrategy - payload: ${JSON.stringify(jwt_payload)}`);

      // Do this check only for the USER apps
      if (type === enums.USER_TYPE.USER) {
        if (isEmpty(date)) {
          logger.error('#JwtStrategy - Property "date" is null. User needs to login again!');
          next(null, false);
        }
      }
      ...
      if (type === enums.USER_TYPE.USER) {
        ...
        if (!isEmpty(environment) && environment !== NODE_ENV && NODE_ENV !== "local") {
      ...
      } else if (type === enums.USER_TYPE.SUPERADMIN) {
      ...
      model
        ...
        .then((object) => {
          if (!object) {
            ...
            if (type === enums.USER_TYPE.SUPERADMIN) {
              object.scope = scope;
              object.type = type;
              next(null, object);
        ...
        .catch((error) => {
          logger.error(`#JwtStrategy - Error encountered: ${error.message}\n${error.stack}`);
          next(null, false);
        });
    })
  );
